                    ...
                    <table className='table-auto border-collapse  border-gray rounded-xl mb-8 w-full mx-auto text-[#495057]'>
                        ...
                        <tbody className='bg-white'>
                            ...
                            <tr key={item.id} className="">
                                ...
                                <td className='border-b border-[#e9ecef]'>{item.nama}</td>
                                <td className='border-b border-[#e9ecef]'>{item.review}</td>
                                {/* <td className='flex items-center border-b p-2 mb-8'><AiFillStar className='fill-yellow'/>{item.stars}</td> */}
                                <td className='border-b border-[#e9ecef] mr-1'>
                                    {item.status === "Pending" ? 
                                ...
                                <td className='flex p-1 justify-center'>
                                    ...
                                    <div className='w-fit mt-1 mb-2'>
                                        <button className=' bg-green text-white rounded-lg font-semibold mb-1 w-16'onClick={()=>{acceptReview(item.id); setPopup(true); setMessages("Published comment berhasil")}}>Accept</button>
                                        <button className=' bg-red text-white rounded-lg font-semibold w-16' onClick={()=> {rejectReview(item.id); setPopup(true); setMessages("Reject comment berhasil")}}>Reject</button>
                                    </div>
                                    : 
                                    null
                                    }
